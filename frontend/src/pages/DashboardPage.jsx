...
const DashboardPage = () => {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-neo-cream p-6 text-neo-ink">
                <div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-[260px_1fr_340px]">
    ...
    return (
        <div className="min-h-screen bg-neo-cream text-neo-ink">
            ...
            <div className="mx-auto grid min-h-screen max-w-[1500px] gap-5 px-4 py-5 lg:grid-cols-[280px_minmax(0,1fr)_360px] lg:px-6">
                <aside className={`fixed inset-y-0 left-0 z-50 w-[285px] overflow-y-auto border-r-4 border-neo-ink bg-white p-4 shadow-brutal transition-transform duration-300 lg:sticky lg:top-5 lg:h-[calc(100vh-2.5rem)] lg:translate-x-0 lg:rounded-[1.35rem] lg:border-4 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-[115%]'}`}>
                ...
                <main className="min-w-0 space-y-5">
                    <header className="neo-card bg-white p-4 md:p-6">
                        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                            <div className="flex items-start gap-3">
                                <button onClick={() => setIsSidebarOpen(true)} className="rounded-2xl border-4 border-neo-ink bg-neo-yellow p-3 font-black lg:hidden">☰</button>
                                <div>
                                    <span className="section-tag">Live SaaS dashboard</span>
                                    <h1 className="mt-4 text-4xl font-black tracking-[-0.05em] md:text-6xl">Pay-per-use AI workspace</h1>
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:min-w-[520px]">
                                ...
                                <div className="rounded-2xl border-4 border-neo-ink bg-neo-yellow p-3 shadow-brutal-sm">
                                ...
                                <div className="rounded-2xl border-4 border-neo-ink bg-neo-pink p-3 shadow-brutal-sm">
                                    <p className="text-xs font-black uppercase">Spent</p>
                                    <p className="text-2xl font-black">{formatAlgo(totalSpent)}</p>
                                    <p className="text-xs font-bold">ALGO</p>
                                </div>
                                <div className="rounded-2xl border-4 border-neo-ink bg-neo-blue p-3 text-white shadow-brutal-sm">
                    ...
                    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
                        <div className="neo-card flex min-h-[620px] flex-col bg-white">
                                ...
                                {messages.length === 0 ? (
                                    <div className="grid h-full place-items-center py-8">
                                        <div className="max-w-2xl text-center">
                                            ...
                                            <h3 className="text-4xl font-black tracking-[-0.04em]">Start the full pay-per-use demo.</h3>
                                            ...
                                            <div className="mt-6 grid gap-3 md:grid-cols-2">
                                                {quickPrompts.map((prompt) => (
                                                    <button key={prompt} onClick={() => setInput(prompt)} className="rounded-2xl border-4 border-neo-ink bg-white p-4 text-left text-sm font-bold shadow-brutal-sm transition-all hover:-translate-y-1 hover:bg-neo-yellow">{prompt}</button>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-5">
                                        {messages.map((msg, idx) => (
                                            <div key={`${msg.role}-${idx}`} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                                <div className={`max-w-[88%] rounded-[1.35rem] border-4 border-neo-ink p-4 shadow-brutal-sm ${msg.role === 'user' ? 'bg-neo-blue text-white' : 'bg-white'}`}>
                            ...
                            <div className="neo-card bg-neo-ink p-5 text-white">
                                ...
                                <div className="mt-4 flex items-end justify-between gap-3">
                                    <div>
                                        ...
                                        <p className="text-5xl font-black tracking-[-0.05em]">{formatAlgo(liveUsage.nextBalance)}</p>
                            ...
                            <div className="neo-card bg-white p-5">
                                ...
                                <div className="mt-4 space-y-3">
                                    {recentTransactions.map((tx) => (
                                        <div key={tx.id} className="flex items-center justify-between rounded-2xl border-3 border-neo-ink bg-neo-cream p-3">
                                            <div>
                                                <p className="font-black">{tx.label}</p>
                                                <p className="text-xs font-bold text-neo-muted">{tx.time}</p>
                                            </div>
                                            <p className={`font-black ${tx.amount < 0 ? 'text-red-600' : 'text-emerald-700'}`}>{tx.amount < 0 ? '-' : '+'}{formatAlgo(Math.abs(tx.amount))}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                <aside className="space-y-5 lg:sticky lg:top-5 lg:h-[calc(100vh-2.5rem)] lg:overflow-y-auto lg:pr-1">
                    <div className="neo-card bg-white p-5">
                        ...
                        <div className="mt-5 flex h-52 items-end gap-3 border-b-4 border-l-4 border-neo-ink p-3">
                            {demoUsage.map((day) => (
                            ...
                            {demoUsage.slice(-5).map((day) => (
                                <div key={day.day}>
                                    ...
                                    <div className="h-4 rounded-full border-3 border-neo-ink bg-white">
                                        <div className="h-full rounded-full bg-neo-pink" style={{ width: `${Math.min(100, day.cost * 100)}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                            ...
                            {modelMix.map((model) => (
                                <div key={model.name}>
                                    ...
                                    <div className="h-5 rounded-full border-3 border-neo-ink bg-neo-cream">
                                        <div className={`h-full rounded-full ${model.color}`} style={{ width: `${model.pct}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="neo-card bg-neo-pink p-5">
                        ...
                        <ol className="mt-4 space-y-3 font-black">
                            {['Sign up / connect wallet', 'Add prepaid balance', 'Send prompt', 'Watch usage + cost', 'Review history'].map((step, index) => (
                                <li key={step} className="flex items-center gap-3 rounded-2xl border-3 border-neo-ink bg-white p-3"><span className="grid h-7 w-7 place-items-center rounded-full bg-neo-ink text-xs text-white">{index + 1}</span>{step}</li>
                            ))}
                        </ol>
                    </div>
                </aside>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neo-ink/60 p-4 backdrop-blur-sm">
                    <div className="neo-card max-w-md bg-white p-6">
                        ...
                        <div className="mt-2 flex items-center gap-4">
                            <div className="grid h-20 w-20 place-items-center rounded-2xl border-4 border-neo-ink bg-neo-yellow text-3xl font-black shadow-brutal-sm">{user?.name?.charAt(0).toUpperCase() || 'U'}</div>
                            <div>
                        ...
                        <div className="mt-6 grid grid-cols-2 gap-3">
                            ...
                            <div className="col-span-2 rounded-2xl border-3 border-neo-ink bg-neo-cream p-3"><p className="text-xs font-black uppercase">Email</p><p className="font-bold">{user?.email || 'Not provided'}</p></div>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isAuthModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neo-ink/60 p-4 backdrop-blur-sm">
                    <div className="neo-card max-w-md bg-white p-7 text-center">
                        ...
                        <h2 className="mt-5 text-3xl font-black">Authorize AI session</h2>
            ...
            {toast && (
                <div className={`fixed bottom-6 right-6 z-[300] rounded-2xl border-4 border-neo-ink px-5 py-4 font-black shadow-brutal animate-slide-up ${toast.type === 'error' ? 'bg-neo-pink' : 'bg-neo-green'}`}>
